/* Sections 1 : Services, Estimateur, Process */
...
function Services() {
  const services = [
  { id: 1, icon: IconBolt, color: "#FFC700",
  ...
  { id: 3, icon: IconCpu, color: "#FFE13B",
    ...
    metric: "+30%", metricLabel: "économies énergie",
    bg: "https://images.unsplash.com/photo-1558002038-1055907df827?w=1200&q=80" },
  { id: 4, icon: IconAward, color: "#FFC700",
  ...
  { id: 5, icon: IconLightbulb, color: "#B6FF3C",
    ...
    metric: "−72%", metricLabel: "facture éclairage",
    bg: "https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=1200&q=80" },
  { id: 6, icon: IconSearch, color: "#FFE13B",
    ...
    metric: "150 €", metricLabel: "forfait audit",
  ...
  return (
    <section id="services" className="section" style={{ background: "var(--bg)" }}>
      <div className="container-wide">
        <SecHead num="01" label="Nos prestations"
        title={<>Six prestations.<br /><span style={{ color: "var(--muted)" }}>Une seule équipe.</span></>}
        ...
        <FadeUp delay={400}>
          <div style={{
            ...
            <div>
              <div className="eyebrow" style={{ marginBottom: 8 }}>Vous ne trouvez pas votre besoin ?</div>
...
function ServiceCard({ icon: Icon, color, title, lead, tags, metric, metricLabel, bg }) {
  ...
  return (
    <div
      ...
      <div style={{ position: "absolute", inset: 0,
        background: "linear-gradient(180deg, rgba(11,14,16,0.30) 0%, rgba(11,14,16,0.45) 45%, rgba(11,14,16,0.92) 100%)",
        pointerEvents: "none" }} />

      {/* Color tint matching the service */}
      <div style={{ position: "absolute", inset: 0,
      ...
      {/* Top row: icon + metric (glassmorphic chips) */}
      <div className="row between center" style={{ padding: 22, position: "relative", zIndex: 2 }}>
        <div style={{
          ...
          boxShadow: hover ? `0 0 24px ${color}55, inset 0 1px 0 rgba(255,255,255,0.20)` : "inset 0 1px 0 rgba(255,255,255,0.15)",
        ...
        <div style={{
          ...
          <span style={{ fontSize: 16, fontWeight: 600, color: "#F5F5F3", fontFamily: "'Inter Tight', sans-serif", letterSpacing: "-0.02em" }}>{metric}</span>
          <span style={{ fontSize: 9.5, fontFamily: "'JetBrains Mono', monospace", color: "rgba(245,245,243,0.7)", textTransform: "uppercase", letterSpacing: "0.05em" }}>{metricLabel}</span>
        </div>
      </div>
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: 19, fontWeight: 500, letterSpacing: "-0.025em", lineHeight: 1.18, marginBottom: 8, color: "#F5F5F3" }}>
          {title}
        </h3>
        ...
        <div className="row" style={{ flexWrap: "wrap", gap: 5 }}>
          ...
          <span key={t} style={{
            ...
            borderRadius: 999,
            fontSize: 10.5, color: "rgba(245,245,243,0.85)", fontFamily: "'JetBrains Mono', monospace"
          }}>
              {t}
            </span>
          )}
...
function Estimateur() {
  ...
  const [type, setType] = useStateS(types[1]);
  const [surface, setSurface] = useStateS(70);
  const [urgent, setUrgent] = useStateS(false);
  ...
  return (
    <section id="estimateur" className="section" style={{ background: "var(--grad-dark-rich)", color: "#F5F5F3", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: "-20%", left: "-10%", width: 600, height: 600,
        background: "radial-gradient(circle, rgba(255,199,0,0.18), transparent 65%)", filter: "blur(80px)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: "-30%", right: "-10%", width: 700, height: 700,
        background: "radial-gradient(circle, rgba(182,255,60,0.14), transparent 65%)", filter: "blur(100px)", pointerEvents: "none" }} />

      <div className="container-wide" style={{ position: "relative" }}>
        <FadeUp>
          <div className="row center gap-12" style={{ marginBottom: 24 }}>
            <span className="num-tag" style={{ color: "var(--accent)" }}>(02)</span>
            <span className="eyebrow" style={{ color: "#F5F5F3" }}>Estimateur</span>
          </div>
          <h2 className="display" style={{ fontSize: "clamp(28px, 3.8vw, 62px)", maxWidth: 880, lineHeight: 0.98, color: "#F5F5F3", marginBottom: 24 }}>
            Estimez votre besoin<br /><span className="grad-text">avant tout rendez-vous.</span>
          </h2>
        ...
        <div className="grid" style={{ gridTemplateColumns: "1.1fr 0.9fr", gap: 32, alignItems: "stretch", marginTop: 56 }}>
          <FadeUp delay={120}>
            <div className="glass" style={{ padding: 36, height: "100%" }}>
              <div className="eyebrow" style={{ color: "rgba(245,245,243,0.6)", marginBottom: 18 }}>Type de prestation</div>
              <div className="row" style={{ flexWrap: "wrap", gap: 8, marginBottom: 36 }}>
                {types.map((t) =>
                <button key={t.id} className="chip" data-active={type.id === t.id}
                onClick={() => setType(t)}
                style={{
                  background: type.id === t.id ? "var(--accent)" : "rgba(255,255,255,0.06)",
                  color: type.id === t.id ? "var(--accent-deep)" : "#F5F5F3",
                  border: type.id === t.id ? "1px solid var(--accent)" : "1px solid rgba(255,255,255,0.10)"
                }}>
                    {t.label}
                  </button>
                )}
              ...
              <>
                  <div className="row between center" style={{ marginBottom: 12 }}>
                    <div className="eyebrow" style={{ color: "rgba(245,245,243,0.6)" }}>Surface · m²</div>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", color: "var(--accent)", fontSize: 18 }}>{surface} m²</div>
                  </div>
                  <input type="range" min="20" max="200" step="5" value={surface} onChange={(e) => setSurface(+e.target.value)}
              ...
              <div className="row between center" style={{
                ...
                borderRadius: 12, border: "1px solid rgba(255,255,255,0.08)", marginBottom: 12
              }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500 }}>Intervention urgente</div>
                  <div style={{ fontSize: 11.5, color: "rgba(245,245,243,0.55)", marginTop: 2 }}>Sous 2 h, week-end inclus · +35%</div>
                </div>
                <button onClick={() => setUrgent(!urgent)} style={{
                  width: 44, height: 24, borderRadius: 999,
                  background: urgent ? "var(--accent)" : "rgba(255,255,255,0.15)",
                  position: "relative", transition: "all 0.3s var(--e-smooth)"
                }}>
                  <span style={{
                    position: "absolute", top: 3, left: urgent ? 23 : 3,
                    width: 18, height: 18, borderRadius: "50%", background: "#0B0E10",
          ...
          <FadeUp delay={240}>
            <div style={{
              ...
              border: "1px solid rgba(255,199,0,0.28)", borderRadius: 28,
              backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)",
              display: "flex", flexDirection: "column", justifyContent: "space-between",
              boxShadow: "0 30px 80px rgba(255,199,0,0.18), 0 0 0 1px rgba(255,255,255,0.08) inset",
              position: "relative", overflow: "hidden"
            }}>
              <div style={{ position: "absolute", top: -100, right: -100, width: 280, height: 280, background: "radial-gradient(circle, rgba(255,199,0,0.4), transparent 65%)", filter: "blur(40px)" }} />
              <div style={{ position: "relative" }}>
                <div className="eyebrow" style={{ color: "rgba(245,245,243,0.7)", marginBottom: 12 }}>Estimation indicative</div>
                ...
                <p style={{ fontSize: 12, color: "rgba(245,245,243,0.5)", fontFamily: "'JetBrains Mono', monospace" }}>
                  {type.unit} {urgent ? "· urgence" : ""}
                </p>
              </div>

              <div style={{ position: "relative", margin: "32px 0" }}>
                <div style={{ fontSize: 110, fontWeight: 600, lineHeight: 0.85, letterSpacing: "-0.05em", color: "#F5F5F3" }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                <p style={{ fontSize: 11, color: "rgba(245,245,243,0.5)", textAlign: "center", marginTop: 12, fontFamily: "'JetBrains Mono', monospace" }}>
...
function Process() {
  ...
  const steps = [
  { num: "01", icon: IconPhone, title: "Prise de contact", body: "Téléphone ou formulaire. On répond sous 24 h ouvrées, souvent dans l'heure. On situe votre besoin, on cale un créneau." },
  { num: "02", icon: IconSettings, title: "Visite & devis chiffré", body: "On vient mesurer, photographier, comprendre. Le devis arrive sous 48 h, ligne par ligne, avec photos avant." },
  { num: "03", icon: IconBolt, title: "Chantier & garantie 2 ans", body: "Démarrage à votre date. On laisse propre, on documente le rendu, on garantit deux ans pièce et main-d'œuvre." }];


  return (
    <section id="process" className="section" style={{ background: "var(--bg)" }}>
      <div className="container-wide">
        <SecHead num="03" label="Process" title={<>Trois étapes.<br /><span style={{ color: "var(--muted)" }}>Et on s'occupe du reste</span></>}
        sub="Du premier appel au coup de balai final, vous savez où vous en êtes. Aucun travail ne démarre sans devis signé." />

        <div ref={ref} className="grid" style={{ gridTemplateColumns: "repeat(3, 1fr)", gap: 24, position: "relative" }}>
          <div className="stepper-line" style={{ "--progress": progress }}></div>
          {steps.map((s, i) =>
          <FadeUp key={s.num} delay={i * 100}>
              <div style={{ position: "relative", zIndex: 2 }}>
                ...
                marginBottom: 24, transition: "all 0.7s var(--e-smooth)",
                boxShadow: progress > i / 3 ? "0 12px 30px rgba(255,199,0,0.35)" : "none"
              }}>
                  <s.icon size={22} />
                </div>
                <div className="num-tag" style={{ marginBottom: 10 }}>{s.num}</div>
                <h3 style={{ fontSize: 28, fontWeight: 500, letterSpacing: "-0.03em", lineHeight: 1.1, marginBottom: 12 }}>{s.title}</h3>
...
Object.assign(window, { Services, Estimateur, Process });