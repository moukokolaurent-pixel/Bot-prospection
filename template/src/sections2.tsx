/* Sections 2 : Realisations, Testimonials (avis_google), Zone (hq toggle), FAQ, Certifs */
import { useEffect, useState } from 'react';
import { FadeUp, SecHead, useProspect } from './shared';
import {
  IconArrowRight,
  IconHome,
  IconMapPin,
...
/* -------------------- Realisations (générique avec note "exemple") -------------------- */

interface ReaCase {
  id: number;
  title: string;
  /** "Castelnau-le-Lez · T4 · 95 m²" → "T4 · 95 m²" après stripVille() */
  lieu: string;
  cat: string;
  durée: string;
  color: string;
...
/** Retire le premier segment ` · ` (ville) du champ `lieu`. */
function stripVille(lieu: string): string {
  const parts = lieu.split(' · ');
  if (parts.length <= 1) return lieu;
...
export function Realisations() {
  const cases: ReaCase[] = [
    {
      id: 1,
      title: 'Tableau électrique vétuste → conforme NF C 15-100',
      lieu: 'Castelnau-le-Lez · T4 · 95 m²',
      cat: 'Mise aux normes',
      durée: '2 jours',
      color: '#FFC700',
    ...
    {
      id: 2,
      title: 'Domotique complète Apple Home',
      lieu: 'Montpellier centre · Maison · 140 m²',
      cat: 'Domotique',
      durée: '5 jours',
      color: '#B6FF3C',
      avant:
        'Volets manuels, chauffage individuel non pilotable, alarme datée, +850 €/an de chauffage.',
      apres:
        '12 volets motorisés, thermostats Netatmo, alarme Somfy, scénarios HomeKit. -32% facture chauffage.',
      url: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=1200&q=80'
    },
    {
      id: 3,
      title: 'Borne de recharge IRVE Wallbox 11 kW',
      lieu: 'Pérols · Maison individuelle',
      cat: 'Borne IRVE',
      durée: '1 jour',
      color: '#FFE13B',
    ...
    {
      id: 4,
      title: 'Éclairage architectural cuisine + salon',
      lieu: 'Lattes · Appartement neuf',
      cat: 'Éclairage LED',
      durée: '3 jours',
      color: '#FFC700',
  ...
  const [open, setOpen] = useState<ReaCase | null>(null);
  ...
  return (
    <section id="realisations" className="section" style={{ background: 'var(--bg-2)' }}>
      <div className="container-wide">
        ...
        >
          Exemples de mise en page · vos vraies réalisations remplaceront ce contenu
        </p>
              ...
              >
                ...
                <div
                  style={{
                    ...
                    inset: 0,
                    background:
                      'linear-gradient(180deg, rgba(11,14,16,0.2) 0%, rgba(11,14,16,0.85) 75%, rgba(11,14,16,0.95) 100%)'
                  }}
                />
...
function BeforeAfterModal({ data, onClose }: { data: ReaCase; onClose: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: 24,
        animation: 'pageIn 0.4s var(--e-smooth)'
      }}
    >
      <div
      ...
      >
        ...
        <div className="grid grid-2" style={{ padding: 28, gap: 24 }}>
          ...
          <div>
            <div className="eyebrow" style={{ marginBottom: 8, color: 'var(--ink)' }}>
              Après
            </div>
...
/* -------------------- Testimonials (avis_google ; min 3 sinon caché) -------------------- */

export function Testimonials() {
  const data = useProspect();
  const items = data.avis_google;
  ...
  // brief §2.3 : section masquée si < 3 avis
  if (items.length < 3) return null;

  const noteStr = data.note_google !== undefined ? data.note_google.toLocaleString('fr-FR') : '—';
  const nbStr = data.nombre_avis !== undefined ? String(data.nombre_avis) : '—';
        ...
        <SecHead
          ...
          title={
            <>
              Ce que disent
              <br />
              <span style={{ color: 'var(--muted)' }}>nos clients.</span>
            </>
          }
          sub={`Avis Google vérifiés. Note moyenne ${noteStr} / 5 sur ${nbStr} avis.`}
        />

        <div className="row center" style={{ gap: 6, marginBottom: 24 }}>
          {[1, 2, 3, 4, 5].map((i) => (
            <IconStar key={i} size={20} color="#FFC700" stroke={1.5} />
          ))}
          <span
            style={{
              marginLeft: 12,
...
/* -------------------- Zone (HQ togglé sur ville_principale si match) -------------------- */
...
function matchesHq(cityName: string, ville: string): boolean {
  const a = normalizeCity(cityName);
  const b = normalizeCity(ville);
...
export function Zone() {
  ...
  const baseCities: ZoneCity[] = [
    { name: 'Montpellier', x: 50, y: 56, count: 42, hq: false },
    { name: 'Saint-Clément', x: 47, y: 42, count: 31, hq: true },
    { name: 'Castelnau', x: 56, y: 53, count: 18, hq: false },
    { name: 'Lattes', x: 53, y: 64, count: 11, hq: false },
    { name: 'Pérols', x: 60, y: 66, count: 7, hq: false },
    { name: 'Béziers', x: 22, y: 70, count: 5, hq: false },
    { name: 'Sète', x: 38, y: 78, count: 3, hq: false }
  ];

  const hqIndex = baseCities.findIndex((c) => matchesHq(c.name, data.ville_principale));
  const cities: ZoneCity[] =
    hqIndex >= 0
      ? baseCities.map((c, i) => ({ ...c, hq: i === hqIndex }))
      : baseCities;
    ...
    >
      <div
        style={{
          ...
          width: 500,
          height: 500,
          background: 'radial-gradient(circle, rgba(255,199,0,0.12), transparent 65%)',
          filter: 'blur(80px)',
          pointerEvents: 'none'
        }}
      />
      ...
      <div className="container-wide" style={{ position: 'relative' }}>
        <div
          className="grid"
          style={{ gridTemplateColumns: '0.85fr 1.15fr', gap: 64, alignItems: 'center' }}
        >
          <FadeUp>
            ...
            >
              Disponibles
              <br />
              <span className="grad-text">dans tout l'Hérault.</span>
            </h2>
            <p
              style={{
                fontSize: 17,
            ...
            >
              Basés à {data.ville_principale}, nous intervenons à Montpellier, Béziers, Sète, et
              toutes les communes de l'agglomération. Pas de frais de déplacement caché — c'est dans
              le devis.
            </p>
                ...
                >
                  <div className="row center gap-12">
                    {c.hq ? (
                      ...
                      <IconMapPin size={14} color="rgba(245,245,243,0.5)" />
                    )}
          ...
          <FadeUp delay={150}>
            ...
            >
              ...
              >
                ...
                <path
                  d="M 12,20 L 28,14 L 42,18 L 55,12 L 68,18 L 78,28 L 82,42 L 78,56 L 70,68 L 62,76 L 48,82 L 32,84 L 18,76 L 10,62 L 8,48 L 10,34 Z"
                  fill="url(#zonefill)"
                  stroke="rgba(255,199,0,0.35)"
                  strokeWidth="0.4"
                  strokeDasharray="1.2 1.2"
                />
                <path
                  d="M 70,76 L 56,82 L 38,84 L 22,82 L 12,72"
                  fill="none"
                  stroke="rgba(245,245,243,0.30)"
                  strokeWidth="0.3"
                />
              </svg>
...
/* -------------------- FAQ (générique, CTA téléphone du prospect) -------------------- */
        ...
        <div className="grid" style={{ gridTemplateColumns: '0.85fr 1.15fr', gap: 80 }}>
          <FadeUp>
            <div style={{ position: 'sticky', top: 100 }}>
              ...
              <h2
                className="display"
                style={{ fontSize: 'clamp(28px, 3.5vw, 56px)', lineHeight: 0.98, marginBottom: 32 }}
              >
                Questions fréquentes
                <br />
                <span style={{ color: 'var(--muted)' }}>Nous répondons à vos besoins.</span>
              </h2>
              <p style={{ fontSize: 16, color: 'var(--muted)', lineHeight: 1.6, marginBottom: 28 }}>
                Une question pas listée ?<br />
                Appelez-nous, on répond direct.
              </p>
              <a href={`tel:${data.telephone_e164}`} className="btn btn-ghost">
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      transition: 'margin 0.4s var(--e-smooth)'
                    }}
                  >
                    <h3
                      style={{
...
export function Certifs() {
  const items = [
    ...
    'Décennale assurée',
    'RGE éligible',
    'Habilitation B1V/BR/BC',
    'Certified Apple Home'
  ];
  return (
    <div
      style={{
